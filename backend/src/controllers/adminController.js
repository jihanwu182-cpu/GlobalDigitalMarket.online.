  ...
  try {
    ...
    return res.status(200).json({
      ...
      dashboard: {
        ...
        totalAccounts: accountsResult.rows[0].total,
        totalTransactions: transactionsResult.rows[0].total,
        pendingTransactions:
          pendingTransactionsResult.rows[0].total,

        completedDeposits: safeNumber(
          depositsResult.rows[0].total
        ),

        completedWithdrawals: safeNumber(
        ...
        pendingKyc: kycResult.rows[0].total,

        totalAccountBalance: safeNumber(
      ...
      user: {
        ...
        account: row.account_id
          ? {
              ...
              deposit: safeNumber(row.deposit),
              profits: safeNumber(row.profits),
              availableBalance: safeNumber(
                row.available_balance
              ),
              bonus: safeNumber(row.bonus),
              referrerBonus: safeNumber(
                row.referrer_bonus
              ),
              buyingPower: safeNumber(
                row.buying_power
              ),
              marginAvailable: safeNumber(
...
const getKycRequests = async (req, res, next) => {
  try {
    ...
    logger.error('Admin KYC error:', error);
    return next(error);
  }
};
...
const updateTransactionStatus = async (
  ...
  try {
    ...
    // Update balance only when the transaction
